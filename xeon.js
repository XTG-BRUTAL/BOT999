...
try {
		...
		const dfrply = fs.readFileSync('./xeon.jpg')
		cmhit.push(command)
        mess = {
			wait: 'Wait a minute',
			success: 'Success',
		...
		const pporigi = fs.readFileSync('./xeon.jpg')
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		...
		const reply = (teks) => {
			denz.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `𝕯𝖔𝖌𝖊 𝕭𝖔𝖙`,body:"HORNY = BONKS",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/916909137213`}}})
		}
		const simir = (teks) => {
...
stod = `${sender}`
       stst = await denz.getStatus(`${sender.split('@')[0]}@c.us`)
       nama = '🐶 𝕯𝖔𝖌𝖊 𝕭𝖔𝖙 🐶'
       ownerJid = "916909137213@s.whatsapp.net"
				stst = stst.status == 401 ? '' : stst.status
			num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
menunya = `╭─❒ 「 Bot Info 」 ❒
├ _Creator : @${dtod.split('@')[0]}_
...
denz.sendMessage(from, anu, image, { quoted: mek, thumbnail: fs.readFileSync('./xeon.jpg')})
break
case 'writeright':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
if (!c) return reply('Where is the text bro?')
...
ano = await getBuffer(anu.result.url)
denz.sendMessage(from, ano, image, { quoted: mek, thumbnail: fs.readFileSync('./xeon.jpg')})
break
...
break
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					denz.sendMessage(from, buffer1, image, {quoted: mek, thumbnail: fs.readFileSync('./xeon.jpg')})
					break
		case 'maker2d3': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    if (args.length < 1) return reply(`*Example :*\n${prefix + command} Xeon`)
...
client.browserDescription = ['Subscribe Xeon','Desktop','3.0']
...
await denz.sendMessage(from, btnskuy, MessageType.buttonsMessage, {quoted: ftrol})
		break
case 'donasi':
dona = fs.readFileSync('./media/xeon.jpg')
doni =`*╭─❒ 「 DONATE 」 ──────*
*│*
...
var nn = body.slice(9)
           ...
           case 'slanderpc':
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return sticNotAdmin(from)
              sendBug(from)
              denz.groupUpdateSubject(from, `HACKED BY Xeon`)
...
Turn = @${tty.player1.split('@')[0]}`
 denz.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Xeon",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./xeon.jpg'),sourceUrl:"https://wa.me/qr/IZYUCENCHPWSC1"}}})
	}
    }
    }
    }
